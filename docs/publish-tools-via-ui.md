...
| --- | --- | --- |
| `mcp-tools/Jamf_Alert_Posture_Summary.kql` | `Jamf_Alert_Posture_Summary` | Executive roll-up of Jamf Protect alert volume by EventType, severity, prevented vs allowed, unique hosts, and top acting processes over the last 7 days. |
| `mcp-tools/Jamf_Prevented_Execution_Hunt.kql` | `Jamf_Prevented_Execution_Hunt` | Surface the most recent prevented executions on macOS endpoints with full process, file, signing, and parent-process context for analyst pivot. |
| `mcp-tools/Jamf_Unsigned_Binary_Activity.kql` | `Jamf_Unsigned_Binary_Activity` | Find unsigned binaries / files seen running on Mac endpoints with hit count, distinct host fan-out, and severity — concentrates unsigned-code risk into one view. |
| `mcp-tools/Jamf_USB_Storage_Activity.kql` | `Jamf_USB_Storage_Activity` | USB mass-storage mount and block events by host, serial, action, and severity — answers data-exfil and removable-media policy questions. |
| `mcp-tools/Jamf_Mac_Endpoint_Risk_Profile.kql` | `Jamf_Mac_Endpoint_Risk_Profile` | ★ Per-host Mac risk roll-up. Synthesizes a 0–100 RiskScore from severity, prevented count, unsigned binaries, Gatekeeper bypass count, and USB events. Highest-value tool — answers "which Macs need attention first?" |
| `mcp-tools/Jamf_Gatekeeper_MRT_Watch.kql` | `Jamf_Gatekeeper_MRT_Watch` | macOS-native protection mechanism activity (Gatekeeper, MRT, ProcessDenied, ProcessPrevented) grouped by EventType with hit count, distinct hosts, and top target processes. |
